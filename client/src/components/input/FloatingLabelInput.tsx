  ...
  return (
    <>
      <div className="relative">
        <input
          type={inputType}
          {...rest} // 👈 includes name, value, onChange, required, min, max, etc.
          className={`${
            newInputClassName
              ? newInputClassName
              : `block px-2.5 pb-2.5 pt-4 w-full text-sm text-gray-900 bg-transparent rounded-lg border border-gray-300 
        ...
        <label
          ...
          className={`${
            newLabelClassName
              ? newLabelClassName
              : `absolute text-sm text-gray-500 duration-300 transform -translate-y-4 scale-75 top-2 z-10 origin-[0]
        ...
        {isPasswordField && rest.value && (rest.value as string).length > 0 && (
          <button
            type="button"
