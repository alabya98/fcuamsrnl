...
interface DeleteEventFormModalProps {
  ...
  onEventDeleted: (message: string) => void;
  refreshKey: () => void;
  isOpen: boolean;
  onClose: () => void;
}
...
const DeleteEventFormModal: FC<DeleteEventFormModalProps> = ({
  ...
  const handleDeleteEvent = async (e: FormEvent) => {
    try {
      ...
      if (!event || !event.event_id) {
        console.error("❌ No event or event_id available");
        return;
      }
      ...
      if (res.status === 200) {
        onEventDeleted(res.data.message);
        refreshKey();
        onClose();
      }
    } catch (error: any) {
      console.error(
        "❌ Unexpected server error occurred during deleting event: ",
        error
      );
    } finally {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} showCloseButton size="medium">
      <form onSubmit={handleDeleteEvent} className="flex flex-col h-full overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto px-3 sm:px-4 py-2 sm:py-3">
          ...
          <div className="bg-gray-50 p-4 rounded-lg space-y-3">
            <div className="grid grid-cols-3 gap-2">
              ...
              <span className="text-sm font-medium text-gray-900 col-span-2">{event.event_name}</span>
            </div>
            <div className="grid grid-cols-3 gap-2">
              <span className="text-xs font-bold text-gray-600 uppercase tracking-wide">Event Type:</span>
            ...
            <div className="grid grid-cols-3 gap-2">
              ...
              <span className="text-sm font-medium text-gray-900 col-span-2">{formatDate(event.event_date)}</span>
            </div>
            <div className="grid grid-cols-3 gap-2">
            ...
            <div className="grid grid-cols-3 gap-2">
              ...
              <span className="text-sm font-medium text-gray-900 col-span-2">{event.venue}</span>
            </div>
            <div className="grid grid-cols-3 gap-2">
