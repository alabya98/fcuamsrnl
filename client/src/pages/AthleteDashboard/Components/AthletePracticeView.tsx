import { useState, useEffect } from "react";
import PracticeScheduleService from "../../../services/PracticeScheduleService";
import type { PracticeScheduleColumns } from "../../../interfaces/PracticeScheduleInterface";

const AthletePracticeView = () => {
  const [practiceSchedules, setPracticeSchedules] = useState<
    PracticeScheduleColumns[]
  >([]);
  const [loading, setLoading] = useState(true);
  const [filter, setFilter] = useState<"all" | "upcoming" | "completed">(
    "upcoming",
  );

  const loadPracticeSchedules = async () => {
    try {
      setLoading(true);
      const response = await PracticeScheduleService.loadPracticeSchedules();
      if (response.status === 200)
        setPracticeSchedules(response.data.practice_schedules);
    } catch (error) {
      console.error("Error loading practice schedules:", error);
    } finally {
      setLoading(false);
    }
  };

  useEffect(() => {
    loadPracticeSchedules();
  }, []);

  const formatTime = (time: string) => (!time ? "" : time.substring(0, 5));

  const isPastDate = (dateString: string) => {
    const scheduleDate = new Date(dateString);
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    return scheduleDate < today;
  };

  const filteredSchedules = practiceSchedules.filter(
    (schedule: PracticeScheduleColumns) => {
      const isCompleted = schedule.status === "Completed";
      const isPast = isPastDate(schedule.practice_date);
      if (filter === "all") return true;
      if (filter === "upcoming") return !isCompleted && !isPast;
      if (filter === "completed") return isCompleted || isPast;
      return true;
    },
  );

  const getStatusColor = (status: string) => {
    switch (status) {
      case "Approved":
        return "bg-green-100 dark:bg-green-500/20 text-green-800 dark:text-green-300";
      case "Pending":
        return "bg-yellow-100 dark:bg-yellow-500/20 text-yellow-800 dark:text-yellow-300";
      case "Completed":
        return "bg-gray-100 dark:bg-white/10 text-gray-800 dark:text-gray-300";
      case "Declined":
        return "bg-red-100 dark:bg-red-500/20 text-red-800 dark:text-red-300";
      default:
        return "bg-blue-100 dark:bg-blue-500/20 text-blue-800 dark:text-blue-300";
    }
  };

  if (loading) {
    return (
      <div className="flex items-center justify-center py-12">
        <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600"></div>
      </div>
    );
  }

  return (
    <div className="bg-white dark:bg-[#1a1f2e] rounded-2xl shadow-sm dark:shadow-black/30 p-6 border border-gray-200 dark:border-white/5 transition-colors duration-300">
      {/* Header */}
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl font-bold text-gray-800 dark:text-white flex items-center gap-3">
          <div className="w-10 h-10 bg-purple-100 dark:bg-purple-500/20 rounded-lg flex items-center justify-center">
            <svg
              className="w-6 h-6 text-purple-600 dark:text-purple-400"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                strokeWidth={2}
                d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"
              />
            </svg>
          </div>
          Practice Sessions
        </h2>

        <div className="flex gap-2">
          {(["all", "upcoming", "completed"] as const).map((f) => (
            <button
              key={f}
              onClick={() => setFilter(f)}
              className={`px-4 py-2 rounded-lg font-medium text-sm transition-all ${
                filter === f
                  ? "bg-purple-600 text-white shadow-md"
                  : "bg-gray-100 dark:bg-white/10 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-white/15"
              }`}
            >
              {f.charAt(0).toUpperCase() + f.slice(1)}
            </button>
          ))}
        </div>
      </div>

      {/* Empty State */}
      {filteredSchedules.length === 0 ? (
        <div className="text-center py-16">
          <div className="w-20 h-20 bg-gray-100 dark:bg-white/10 rounded-full flex items-center justify-center mx-auto mb-4">
            <svg
              className="w-10 h-10 text-gray-400 dark:text-gray-500"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                strokeWidth={2}
                d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"
              />
            </svg>
          </div>
          <p className="text-gray-500 dark:text-gray-400 font-medium text-lg">
            No practice sessions found
          </p>
          <p className="text-gray-400 dark:text-gray-500 mt-1">
            Check back later for new schedules
          </p>
        </div>
      ) : (
        <div className="space-y-4">
          {filteredSchedules.map((schedule: PracticeScheduleColumns) => {
            const isCompleted =
              schedule.status === "Completed" ||
              isPastDate(schedule.practice_date);

            return (
              <div
                key={schedule.practice_schedule_id}
                className={`bg-gradient-to-r from-gray-50 to-white dark:from-white/5 dark:to-[#1e2433] border-2 rounded-xl p-5 hover:shadow-lg dark:hover:shadow-black/20 transition-all duration-200 ${
                  isCompleted
                    ? "border-gray-300 dark:border-white/10 opacity-75"
                    : "border-gray-200 dark:border-white/5 hover:border-purple-300 dark:hover:border-purple-500/40"
                }`}
              >
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-3">
                      <span className="px-4 py-1.5 bg-purple-100 dark:bg-purple-500/20 text-purple-800 dark:text-purple-300 text-sm font-bold rounded-full shadow-sm">
                        {schedule.sport}
                      </span>
                      <span
                        className={`px-4 py-1.5 text-sm font-bold rounded-full shadow-sm ${getStatusColor(isCompleted && schedule.status !== "Completed" ? "Completed" : schedule.status)}`}
                      >
                        {isCompleted && schedule.status !== "Completed"
                          ? "Completed"
                          : schedule.status}
                      </span>
                    </div>

                    <h3 className="font-bold text-gray-800 dark:text-white text-xl mb-2">
                      {schedule.venue}
                    </h3>

                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
                      <div className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
                        <svg
                          className="w-4 h-4 text-blue-500 dark:text-blue-400"
                          fill="none"
                          stroke="currentColor"
                          viewBox="0 0 24 24"
                        >
                          <path
                            strokeLinecap="round"
                            strokeLinejoin="round"
                            strokeWidth={2}
                            d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"
                          />
                        </svg>
                        <span className="font-medium">
                          Coach: {schedule.coach?.first_name}{" "}
                          {schedule.coach?.last_name}
                        </span>
                      </div>
                      <div className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
                        <svg
                          className="w-4 h-4 text-green-500 dark:text-green-400"
                          fill="none"
                          stroke="currentColor"
                          viewBox="0 0 24 24"
                        >
                          <path
                            strokeLinecap="round"
                            strokeLinejoin="round"
                            strokeWidth={2}
                            d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"
                          />
                        </svg>
                        <span className="font-medium">
                          {schedule.total_players} players expected
                        </span>
                      </div>
                    </div>

                    {schedule.notes && (
                      <div className="mt-3 p-3 bg-purple-50 dark:bg-purple-500/10 rounded-lg border border-purple-200 dark:border-purple-500/20">
                        <p className="text-sm text-purple-800 dark:text-purple-300">
                          <span className="font-semibold">Note:</span>{" "}
                          {schedule.notes}
                        </p>
                      </div>
                    )}

                    {schedule.admin_notes && (
                      <div className="mt-3 p-3 bg-blue-50 dark:bg-blue-500/10 rounded-lg border border-blue-200 dark:border-blue-500/20">
                        <p className="text-sm text-blue-800 dark:text-blue-300">
                          <span className="font-semibold">Admin Note:</span>{" "}
                          {schedule.admin_notes}
                        </p>
                      </div>
                    )}
                  </div>

                  {/* Date Badge */}
                  <div className="text-right ml-6">
                    <div className="w-20 h-20 bg-gradient-to-br from-purple-100 to-purple-200 dark:from-purple-500/20 dark:to-purple-500/30 rounded-xl flex flex-col items-center justify-center shadow-md">
                      <span className="text-xs text-purple-600 dark:text-purple-300 font-medium">
                        {new Date(schedule.practice_date).toLocaleDateString(
                          "en-US",
                          { month: "short" },
                        )}
                      </span>
                      <span className="text-2xl font-bold text-purple-700 dark:text-purple-200">
                        {new Date(schedule.practice_date).getDate()}
                      </span>
                      <span className="text-xs text-purple-600 dark:text-purple-300 font-medium">
                        {new Date(schedule.practice_date).getFullYear()}
                      </span>
                    </div>
                    <p className="text-sm text-gray-600 dark:text-gray-400 mt-3 font-medium">
                      {formatTime(schedule.start_time)} -{" "}
                      {formatTime(schedule.end_time)}
                    </p>
                  </div>
                </div>
              </div>
            );
          })}
        </div>
      )}
    </div>
  );
};

export default AthletePracticeView;
